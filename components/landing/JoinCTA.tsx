...
import type { JoinCTAContent } from "@/lib/types/page-content";
import Button from "@/components/ui/Button";
import SeamlessVideo from "@/components/ui/SeamlessVideo";

export default function JoinCTA({ content }: { content?: JoinCTAContent | null }) {
  ...
  return (
    <section className="relative py-32 md:py-40 px-6 overflow-hidden bg-bg">
      ...
      <div className="absolute inset-0">
        <SeamlessVideo
          src={content?.video_url ?? "/videos/cta-bg.mp4"}
      ...
      <div ref={ref} className="relative z-10 max-w-3xl mx-auto text-center">
        ...
        >
          {content?.headline_1 ?? "Your next chapter"}
          <br />
          starts <span className="text-primary">{content?.headline_2 ?? "here"}</span>.
        </motion.h2>
        ...
        >
          {content?.subheadline ?? "Builders, designers, and creators in Malaysia\u0027s most active Solana community. Join the Telegram, start contributing, and grow with us."}
        </motion.p>
        ...
        >
          <Button href={content?.cta_1_url ?? SOCIAL_LINKS.telegram} size="lg">
            ...
            {content?.cta_1_text ?? "Join Telegram"}
          </Button>
          <Button href={content?.cta_2_url ?? SOCIAL_LINKS.twitter} variant="ghost" size="lg">
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
            {content?.cta_2_text ?? "Follow on X"}
          </Button>
        </motion.div>
      </div>
    </section>
