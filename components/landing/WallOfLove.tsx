...
function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  ...
  const cardContent = (
    <div className={`h-full bg-gradient-to-br from-primary/20 via-card-border to-primary/10 rounded-2xl p-px group-hover/card:from-primary/40 group-hover/card:via-primary/20 group-hover/card:to-primary/20 transition-all duration-500`}>
      <div className="bg-bg/90 backdrop-blur-sm rounded-2xl p-6 h-full flex flex-col group-hover/card:-translate-y-1.5 transition-transform duration-500 ease-out relative overflow-hidden">
  ...
  if (isTweet) {
    return (
      <a
        ...
        className={`${SIZE_WIDTHS[testimonial.size]} flex-shrink-0 group/card block`}
      >
        {cardContent}
      </a>
    );
